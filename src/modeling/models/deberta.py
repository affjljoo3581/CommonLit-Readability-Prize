...
class DebertaClassificationHead(AttentionBasedClassificationHead):
    """
    A classification head for Deberta model. This class inherits
    `AttentionBasedClassificationHead` class and automatically finds necessary
    hyperparameters from `DebertaConfig`.

    Args:
        config: The dictionary-based configuration object (`DebertaConfig`).
    """
...
class DebertaForCustomSequenceClassification(
    TransformerMixin,
    ClassifierMixin,
    DebertaPreTrainedModel,
):
    """A custom sequence classification model with Deberta.

    Compared to the original version (`DebertaForSequenceClassification`), this model
    ...
    understanding of the internal implementations of this and deberta model.
    ...
    def init_transformer_layers(self, num_layers: int, reverse: bool = True):
        if reverse:
            target_layers = self.deberta.encoder.layer[: -num_layers - 1 : -1]
        else:
            target_layers = self.deberta.encoder.layer[:num_layers]
    ...
    def hierarchical_modules(self) -> Iterator[Iterator[nn.Module]]:
        if hasattr(self.deberta.encoder, "rel_embeddings"):
            yield itertools.chain(
                self.deberta.embeddings.modules(),
                self.deberta.encoder.rel_embeddings.modules(),
            )
        else:
            yield self.deberta.embeddings.modules()

        yield from [layer.modules() for layer in self.deberta.encoder.layer]
