...
def test_roberta_for_custom_sequence_classification_forwarding(num_labels, return_dict):
...
def test_roberta_for_custom_sequence_classification_modifying_dropout(dropout_prob):
...
def test_roberta_for_custom_sequence_classification_initializing_classifier():
...
def test_roberta_for_custom_sequence_classification_initializing_transformer_layers(
    num_layers, reverse
):
    initialized_modules = set()
...
def test_roberta_for_custom_sequence_classification_covering_all_parameters():
    config = AutoConfig.from_pretrained("roberta-base")
    model = RobertaForCustomSequenceClassification(config)
