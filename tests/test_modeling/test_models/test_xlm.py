...
def test_xlm_for_custom_sequence_classification_forwarding(num_labels, return_dict):
    config = AutoConfig.from_pretrained("xlm-mlm-ende-1024", num_labels=num_labels)
    ...
    model = XLMForCustomSequenceClassification(config)
...
def test_xlm_for_custom_sequence_classification_modifying_dropout(dropout_prob):
    config = AutoConfig.from_pretrained("xlm-mlm-ende-1024")
    model = XLMForCustomSequenceClassification(config)
...
def test_xlm_for_custom_sequence_classification_initializing_classifier():
...
def test_xlm_for_custom_sequence_classification_initializing_transformer_layers(
    num_layers, reverse
):
    initialized_modules = set()
...
def test_xlm_for_custom_sequence_classification_covering_all_parameters():
    config = AutoConfig.from_pretrained("xlm-mlm-ende-1024")
    model = XLMForCustomSequenceClassification(config)
